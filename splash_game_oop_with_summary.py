...
class SplashGame:
    def __init__(
        ...
        *,
        ...
        property_cost: Optional[dict[str, int]] = None,
        ...
        self.property_cost = dict(property_cost or PROPERTY_COST)
        ...
        self.history = []
        self.years_with_no_money_for_decisions = 0
        self.no_money_blocked_decision_this_year = False
    ...
    @property
    def total_original_town_value(self) -> int:
        return sum(self.property_cost[property_name] for property_name in self.properties)

    @property
    def total_town_value_left(self) -> int:
        """
        Total value of buildings that are still functional.
        Damaged buildings count as $0 value.
        """
        return sum(
            self.property_cost[property_name]
            for property_name in self.properties
    ...
    def ask_and_spend(self, question: str, cost: int) -> bool:
        ...
        if cost > self.budget:
            ...
            self.no_money_blocked_decision_this_year = True
            return False
    ...
    def run_one_year(self):
        self.year += 1
        starting_budget = self.budget
        starting_population = self.total_population
        starting_town_value = self.total_town_value_left
        self.no_money_blocked_decision_this_year = False
        ...
        if self.no_money_blocked_decision_this_year:
            self.years_with_no_money_for_decisions += 1

        record = {
            "year": self.year,
            "starting_budget": starting_budget,
            "starting_population": starting_population,
            "ending_population": self.total_population,
            "population_change": self.total_population - starting_population,
            "starting_town_value": starting_town_value,
            "ending_town_value": self.total_town_value_left,
            "town_value_lost": self.total_original_town_value - self.total_town_value_left,
            "no_money_blocked_decision_this_year": self.no_money_blocked_decision_this_year,
            "years_with_no_money_for_decisions": self.years_with_no_money_for_decisions,
            ...
            rows.append(
                {
                    ...
                    "starting_population": record["starting_population"],
                    "ending_population": record["ending_population"],
                    "population_change": record["population_change"],
                    "remaining_population": record["ending_population"],

                    # Value left in town
                    "starting_town_value": record["starting_town_value"],
                    "ending_town_value": record["ending_town_value"],
                    "town_value_lost": record["town_value_lost"],
                    ...
                    # Years where budget prevented decisions
                    "no_money_blocked_decision_this_year": record[
                        "no_money_blocked_decision_this_year"
                    ],
                    "years_with_no_money_for_decisions": record[
                        "years_with_no_money_for_decisions"
                    ],
    ...
    def final_summary_df(self) -> pd.DataFrame:
        """
        One-row table showing the final result after all simulated years.
        """
        return pd.DataFrame(
            [
                {
                    "years_simulated": self.year,
                    "original_town_value": self.total_original_town_value,
                    "value_left_in_town": self.total_town_value_left,
                    "value_lost": self.total_original_town_value - self.total_town_value_left,
                    "remaining_population": self.total_population,
                    "years_with_no_money_for_decisions": (
                        self.years_with_no_money_for_decisions
                    ),
                    "ending_budget": self.budget,
                    "functional_buildings": len(self.functional_properties),
                    "damaged_properties_remaining": len(self.damaged_properties),
                }
            ]
        )
...
if __name__ == "__main__":
    ...
    years_to_simulate = 3
    summary = game.run_years(years_to_simulate)
    ...
    print()
    print("Final summary:")
    print(game.final_summary_df().to_string(index=False))

    print()
    print("Current state:")
    print(game.current_state_df().to_string(index=False))

    print()
    print("Current mitigation state:")
    print(game.mitigation_state_df().to_string(index=False))
